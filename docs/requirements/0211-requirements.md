...
- 根据社会域中的项目管理员增加和删除资源的请求，调整其对应项目的项目限额（见项目管理员）
...
用户的社会项目在注册时会给予一统一的资源限额（目前确定为4 CPU, 4096 MB, 40 GB)。在社会域中的用户可以在这个限额里创建项目。社会项目不能增加或者减少资源限额。社会项目在计费周期结算时，不使用限额而使用实际使用的资源量进行结算。
