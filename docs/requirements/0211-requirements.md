...
云管理员在创建域的时候，会创建一个**域默认管理员**。**域默认管理员**用户必须不属于任何一个项目，由系统管理员设置。

**域管理员** 用户可以（包括**域默认管理员**，除了社会域管理员，见下）：
- 查询域下的用户
- 给域下的用户设置成域管理员（除了域默认管理员，其他域管理员可以加入项目）
- 设置域的付款用户
- 增删改查域下的项目
- 给用户分配到项目中并指定角色
- 对项目分配资源
...
- 用户管理（Identity）
  - 账号管理（Account，修改邮箱、密码）
  - 系统管理（System，增删改查系统用户）
  - 域管理（Domains，增删改查域，设置域默认管理员账号和密码，给域分配资源）
- 费用管理（Expenses）
  - 总览（Overview，显示账号余额，提供充值入口；显示域应支，可以跳到Domain部分）
  - 交易记录（Transactions，提供当前账号的交易信息：时间、交易额、原因）
  - 系统计费（System，显示和系统有关（给系统交的费用）的交易记录：时间、支付人、费用、原因（管理费、Domain资源支出））
  - 域计费（Domains，分为分配和使用，分开显示）
    - 分配（Allocated，显示各个**域**当前**计费周期**中**分配的资源**、**费率**、**当前支付人**、**上次扣款的时刻**和**支付人**。可以选择一个域，显示之前每个**计费周期**域**分配的资源**、**费用**、**支付人**）
    - 使用（Used，显示各个**域**当前**使用周期**中**使用的资源**、**费率**。可以选择一个**域**，显示之前每个**使用周期**域**使用的资源**和**费用**）


## 3.2 域管理员（Domain Admin）

- 用户管理（Identity）
  - 账号管理（Account，修改学号、邮箱、密码）
  - 项目管理（Projects，增删改查项目，修改给项目分配的资源）
  - 用户管理（Users，查询域中的用户，从域中删除用户，给用户分配到项目中并指定角色，给域中的用户设定域的角色）
- 费用管理（Expenses）
  - 总览（Overview，显示账号余额，提供充值入口；显示域当前计费周期的信息，可以跳到域计费部分；显示各个项目当前计费周期信息的列表（见下项目计费/分配），可以跳到项目计费部分）
  - 交易记录（Transactions，提供**当前账号的交易信息**：时间、交易额、原因，和**和当前域有关（项目给域钱、域给云钱，管理费）的交易记录**：时间、支付人、接收人、费用、原因（管理费、项目资源收入、域资源支出））
  - 域计费（Domain）
  ...
  - 项目计费（Project）
    - 分配（Allocated，显示各个项目当前**计费周期**中**分配的资源**、**费率**、**当前支付人**、**上次扣钱的时刻**和**支付人**。可以选择一个**项目**，显示之前每个**计费周期**项目**分配的资源**、**费用**、**支付人**）
...
- 用户管理（Identity）
  - 账号管理（Account，修改学号、邮箱、密码）
  - 项目管理（Projects，显示域中的所有项目，只读，对社会域项目管理员隐藏)
  - 用户管理（Users，查询域中的用户，向项目中增加用户，从项目中删除用户，给项目中的用户设定项目的角色，给项目中的用户限制可以使用的资源，对社会域项目管理员隐藏）
- 费用管理（Expenses）
  - 总览（Overview，显示账号余额，提供充值入口；显示项目当前计费周期的信息（见下Project/Allocated），可以跳到Project部分；显示各个用户当前计费周期信息的列表（见下Users/Allocated），可以跳到Users部分）
  - 交易记录（Transactions，提供**当前账号的交易信息**：时间、交易额、原因，和**和当前项目有关（项目给域钱，管理费）的交易记录**：时间、支付人、接收人、费用、原因（管理费、项目资源支出）））
  - 项目计费（Project）
...
- 资源管理（Resources）
  - 总览（Overview，个人目前已经使用的资源和被分配的资源）
  - 实例管理
  - 云硬盘管理
  - 网络管理
    - 私有网络
...
- 用户管理（Identity）
  ...
  - 用户管理（Users，只显示当前project的，只读）
- 费用管理（Expenses）
  - 总览（Overview，显示账号余额，提供充值入口）
  - 交易记录（Transactions，提供当前账号的交易信息：时间、交易额、原因（管理费））
  - 用户计费（User）
...
- 资源管理（Resources）
  - 总览（Overview，个人已经使用的资源和被分配的资源，资源使用记录，参考horizon的dashboard界面）
  - 实例管理
  - 云硬盘管理
  - 网络管理
    - 私有网络
...
- 费用管理（Expenses）
  - 总览（Overview，显示账号余额，提供充值入口）
  - 交易记录（Transactions，提供当前账号的交易信息：时间、交易额、原因（管理费）、余额）